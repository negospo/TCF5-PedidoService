...
namespace API
{
    ...
    {
        static string PrefixPath => UsePrefixPath ? "/pedido" : string.Empty;
        ...
        static void ConfigBuilder(WebApplicationBuilder builder)
        {
            ...
            builder.Services.AddTransient<Application.Interfaces.UseCases.IPedidoStatusUseCase, Infrastructure.Status.Service>();
            builder.Services.AddTransient<Application.Interfaces.UseCases.IPedidoPagamentoUseCase, Infrastructure.Pagamento.Service>();

            builder.Services.AddTransient<Application.Interfaces.RabbitMQ.IPedidoPagamentoMessage, Infrastructure.RabbitMQ.PedidoPagamento>();
            builder.Services.AddTransient<Application.Interfaces.RabbitMQ.IPedidoStatusMessage, Infrastructure.RabbitMQ.PedidoStatus>();

            builder.Services.AddSingleton <Application.Interfaces.RabbitMQ.IPedidoPagamentoMessageHandler, Application.Implementations.PedidoPagamentoMessageHandler> ();
            builder.Services.AddHostedService<Listener>(serviceProvider =>
            {
                var messageHandler = serviceProvider.GetRequiredService<IPedidoPagamentoMessageHandler>();
                return new Listener(Settings.RabbitMQConnectionString, "order-payment-status", messageHandler);
            });
        ...
        static void ConfigSwagger(WebApplicationBuilder builder)
        {
            ...
            {
                doc.SwaggerDoc("v1", new OpenApiInfo { Title = "API Pedido", Version = "v1" });
